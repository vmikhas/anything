    ...
    return (
        <section className={'intro'}>
            <TransitionGroup component={null}>
                <CSSTransition key={'title'} classNames={'intro__title'} timeout={300} appear>
                    <h2 className={`intro__title intro__title_${currentPage}`}>{parse(currentContent.title)}</h2>
                </CSSTransition>
                <CSSTransition key={'text'} classNames={'intro__text'} timeout={300} appear>
                    <div className={'intro__text'}>{parse(currentContent.text)}</div>
                </CSSTransition>
                {currentContent?.list && currentContent.list.map((item, index) => (
                    item?.image && (
                        <CSSTransition key={`image-${index}`} classNames={'intro__image'} timeout={300} appear>
